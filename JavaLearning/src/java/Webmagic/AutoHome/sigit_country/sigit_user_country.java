 ...
 *On 2018/4/27  ***13:28.
 ******/
public class sigit_user_country {

    @Test//将省份连接上景区字符串
    public void get_province() throws Exception{

        try {
            File file = new File("D:\\workspace\\java\\WebAnimationTest\\src\\main\\java\\Webmagic\\AutoHome\\sigit_country\\sigits_country.txt");
            List<String> brand_id = FileUtils.readLines(file);
            List<String> list_sigit = new ArrayList<>();
            int jj = 0;
            String province = "";
            for(String brand_id_fil: brand_id) {
                String sig = brand_id_fil + "\t\t" + "gg";
                    list_sigit.add(sig);
            }
            File file1 = new File("D:\\workspace\\java\\WebAnimationTest\\src\\main\\java\\Webmagic\\AutoHome\\sigit_country\\省市_5A_country.txt");
