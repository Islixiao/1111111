...
public class ExcelUtil {
    Log log = LogFactory.getLog(mongoDBConnect.ExcelTest.ExcelUtil.class);
...
//                Iterator<Cell> itc = row.cellIterator();
    ...
    private static void readOne(Map<String, List<List<String>>> result, String file, int headLines, int initLine, int initColumn) {
        try {
            POIFSFileSystem fs = new POIFSFileSystem(mongoDBConnect.ExcelTest.ExcelUtil.class.getResourceAsStream(file));
            HSSFWorkbook workBook = new HSSFWorkbook(fs);
