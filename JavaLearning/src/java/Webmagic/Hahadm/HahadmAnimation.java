package Webmagic.Hahadm;

/***Created by moyongzhuo
 *On 2017/10/21  ***19:37.
 ******/
public class HahadmAnimation {
    private int id;
    private String hahawebname;// 标题
    private String tag;//类别
    private String year;//播放时间
    private String director;//导演
    private String content;//剧情简介
    private float goal;//评分
    private Integer mentotal;//评分人数
    private String original;//原作
    private String screenwriter;//编剧
    private String allname;//别名
    private String dubbing;//配音
    private String role;//配音
    private String area;//地区
    private String languge;//语言
    private String url;//数据来自
    private String vediourl;//视频地址
    private String hhimageurl;//图像地址
    private String contentdetail;//剧情简介











    @Override
    public String toString() {
        return "HahadmAnimation{" +
                "id=" + id +
                ", hahawebname='" + hahawebname + '\'' +
                ", tag='" + tag + '\'' +
                ", year='" + year + '\'' +
                ", director='" + director + '\'' +
                ", content='" + content + '\'' +
                ", goal=" + goal +
                ", mentotal=" + mentotal +
                ", original='" + original + '\'' +
                ", screenwriter='" + screenwriter + '\'' +
                ", allname='" + allname + '\'' +
                ", dubbing='" + dubbing + '\'' +
                ", role='" + role + '\'' +
                ", area='" + area + '\'' +
                ", languge='" + languge + '\'' +
                ", url='" + url + '\'' +
                ", vediourl='" + vediourl + '\'' +
                ", hhimageurl='" + hhimageurl + '\'' +
                ", contentdetail='" + contentdetail + '\'' +
                '}';
    }













    public String getUrl() {
        return url;
    }

    public void setUrl(String url) {
        this.url = url;
    }

    public String getVediourl() {
        return vediourl;
    }

    public void setVediourl(String vediourl) {
        this.vediourl = vediourl;
    }

    public String getHhimageurl() {
        return hhimageurl;
    }

    public void setHhimageurl(String hhimageurl) {
        this.hhimageurl = hhimageurl;
    }

    public String getContentdetail() {
        return contentdetail;
    }

    public void setContentdetail(String contentdetail) {
        this.contentdetail = contentdetail;
    }








    public int getId() {
        return id;
    }

    public void setId(int id) {
        this.id = id;
    }

    public String getHahawebname() {
        return hahawebname;
    }

    public void setHahawebname(String hahawebname) {
        this.hahawebname = hahawebname;
    }

    public String getTag() {
        return tag;
    }

    public void setTag(String tag) {
        this.tag = tag;
    }

    public String getYear() {
        return year;
    }

    public void setYear(String year) {
        this.year = year;
    }

    public String getDirector() {
        return director;
    }

    public void setDirector(String director) {
        this.director = director;
    }

    public String getContent() {
        return content;
    }

    public void setContent(String content) {
        this.content = content;
    }

    public float getGoal() {
        return goal;
    }

    public void setGoal(float goal) {
        this.goal = goal;
    }

    public Integer getMentotal() {
        return mentotal;
    }

    public void setMentotal(Integer mentotal) {
        this.mentotal = mentotal;
    }

    public String getOriginal() {
        return original;
    }

    public void setOriginal(String original) {
        this.original = original;
    }

    public String getScreenwriter() {
        return screenwriter;
    }

    public void setScreenwriter(String screenwriter) {
        this.screenwriter = screenwriter;
    }

    public String getAllname() {
        return allname;
    }

    public void setAllname(String allname) {
        this.allname = allname;
    }

    public String getDubbing() {
        return dubbing;
    }

    public void setDubbing(String dubbing) {
        this.dubbing = dubbing;
    }

    public String getRole() {
        return role;
    }

    public void setRole(String role) {
        this.role = role;
    }

    public String getArea() {
        return area;
    }

    public void setArea(String area) {
        this.area = area;
    }

    public String getLanguge() {
        return languge;
    }

    public void setLanguge(String languge) {
        this.languge = languge;
    }






}
